...
function setTheme(themeName) {
    ...
    //非默认主题
    result = changeBg("url('https://cdn.ichika.cc/theme/" + themeName + "/bg.jpg')", 1);
    if (!result) changeBg("url('https://cdn.ichika.cc/theme/" + themeName + "/bg.png')", 1);
    try {
        pjaxSelectors.forEach(function (item, index, arr) {
            if (item === "#web_bg") {
                arr.splice(index, 1);
...
function getFestival() {
    ...
    // 公历节日
    const gregorianCalendarFestivals = {
        // '01-01': 'NewYear',
        // '01-02': 'NewYear',
        // '01-03': 'NewYear',
    ...
    if (gregorianCalendarFestivals.hasOwnProperty(todayFormatted)) {
        return gregorianCalendarFestivals[todayFormatted];
    }
    ...
    const lunarDateFormatted = `${lunarDate.month.toString().padStart(2, '0')}-${lunarDate.day.toString().padStart(2, '0')}`;

    // 农历节日
    const lunarCalendarFestivals = {
        // '01-01': '春节',
        // '01-02': '春节',
        // '01-03': '春节',
        // '01-15': '元宵节',
    ...
    if (lunarCalendarFestivals.hasOwnProperty(lunarDateFormatted)) {
        return lunarCalendarFestivals[lunarDateFormatted];
    }
