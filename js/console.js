...
function showBgSetting () { //切换背景页面
    const fesTheme=['2022·圣诞','2023·元旦']
...
function layoutSwitch(name, flag) {
    ...
    switch (name) {
        ...
        case 'bgBlur':
            if (switchOn) {
                if (document.documentElement.getAttribute('data-theme') === 'dark') document.documentElement.style.setProperty('--ichika-card-bg', 'rgba(0,0,0,0.7)')
                else document.documentElement.style.setProperty('--ichika-card-bg', 'rgba(255,255,255,0.75)')
                document.documentElement.style.setProperty('--ichika-bgblur', 'blur(10px)')
            }
            else {
                if (document.documentElement.getAttribute('data-theme') === 'dark') document.documentElement.style.setProperty('--ichika-card-bg', 'rgba(0,0,0,0.7)')
                else document.documentElement.style.setProperty('--ichika-card-bg', 'rgba(255,255,255,0.95)')
                document.documentElement.style.setProperty('--ichika-bgblur', 'blur(0px)')
            }
            break;
        case 'useSakura':
            stopp(switchOn)
...
} catch (error) { saveData('autoTheme', isFestival); console.log('error1')}
